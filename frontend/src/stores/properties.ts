...
import {
  searchProperties, createProperty, updateProperty, deleteProperty, createReservation,
...
export const usePropertyStore = defineStore("properties", {
  ...
  actions: {
    ...
    async fetchListByBounds(lat: number, lon: number, radiusKm: number) {
      ...
      this.error = null
      try {
        const res = await searchByBounds({ lat, lon, radiusKm })
        this.list = res.data
      } catch (err) {
...
async toggleFavourite(idProp: number) {
  ...
  if (isFav) {
    ...
    async fetchImages(id: number): Promise<string[]> {
      try {
        return await getPropertyImages(id)
      } catch (e) {
