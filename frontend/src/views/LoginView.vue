...
function parseAxiosError(err: unknown, fallback: string) {
  ...
  if (err.response?.status === 400) return "Richiesta non valida."
  if (err.response?.status === 401) return "Email o password non validi."
  if (err.response?.status === 403) return "Accesso non consentito."
  if (err.response?.status === 500) return "Errore del server. Riprova più tardi."
...
const handleLogin = async () => {
  ...
  try {
    ...
    error.value = parseAxiosError(err, "Errore durante il login. Riprova.")
  } finally {
    loading.value = false
  }
}
...
const loginWithFacebook = () => {
  ...
  const params = new URLSearchParams({ auth_type: "reauthenticate" })
  window.location.href = `${baseUrl}?${params.toString()}`
}
