...
function parseRegisterError(err: unknown) {
  ...
  if (err.response?.status === 400) {
    return "Dati non validi. Controlla nome, email, password e telefono."
  }
...
const handleRegister = async () => {
  ...
  if (!form.name.trim()) {
    error.value = "Inserisci il nome completo."
    registering.value = false
    return
  }
  ...
  if (!passwordRegex.test(form.password)) {
    error.value =
      "La password deve avere almeno 8 caratteri, un numero e un carattere speciale."
    registering.value = false
    return
  }
  ...
  try {
    ...
    if (typeof data === "string" && data.trim()) {
      ...
      error.value =
        "Dati non validi. Controlla nome, email e password: la password deve contenere almeno 8 caratteri, un numero e un carattere speciale."
    } else {
